...
WHERE m.user_id IS NULL AND u.email IS NOT NULL;

-- Ensure all users have manager role
INSERT INTO public.user_roles (user_id, role)
SELECT u.id, 'manager'::app_role
FROM auth.users u
LEFT JOIN public.user_roles ur ON ur.user_id = u.id AND ur.role = 'manager'::app_role
WHERE ur.user_id IS NULL AND u.email IS NOT NULL;